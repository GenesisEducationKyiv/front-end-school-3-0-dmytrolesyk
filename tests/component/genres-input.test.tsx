...
test('GenresTagInput component test', async ({ mount, page }) => {
  ...
  await expect(component.getByTestId('badge').getByText('Reggae')).not.toBeVisible();

  await component.getByRole('combobox').click();
  await expect(page.getByText('Reggae')).toBeVisible();
  await page.getByText('Reggae').click();

  await expect(component.getByTestId('badge').getByText('Reggae')).toBeVisible();
  await component.getByRole('button').getByText('Jazz').click();
  await expect(component.getByTestId('badge').getByText('Jazz')).not.toBeVisible();
});
